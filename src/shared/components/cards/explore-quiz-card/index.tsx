...
export const ExploreQuizCard = ({
  ...
  return (
    <div
      className={cn(
        'w-full relative bg-surface-1 flex flex-col gap-[16px] pt-[20px] pb-[16px] border-b border-divider cursor-pointer',
        className,
      )}
      {...props}
    >
  ...
  return (
    <div className="flex justify-between px-[16px]">
      <div className="flex gap-[8px]">
        <div className="w-6 h-8 text-center text-2xl leading-7">{emoji}</div>
      ...
      {!isOwner && (
        ...
        >
          {isBookmarked ? (
            ...
            <IcBookmark className="size-[20px] text-icon-sub" />
          )}
        </button>
      )}
    </div>
        ...
        {isFetching ? (
          ...
          quizzes.map((quiz) => (
            <div
              key={`quiz-${quiz.id}`}
              className={cn(
                'bg-base-2 py-[12px] px-[16px] w-[280px] h-[64px] rounded-[12px] flex-center flex-shrink-0',
              )}
            >
